...
class GeneticAlgorithm:
        ...
        for _ in range(POPULATION_SIZE):
            ...
            for food in self.food_items:
                if food["unit_category"] == "Base Units":
                    qty = random.uniform(
                        1, 200
                    )  # Initialize with quantities between 1g and 200g for Base Units
                else:
                    qty = random.randint(
                        1, 20
                    )  # Initialize with integer quantities for other units
    ...
    def fitness(self, chromosome):
        ...
        for gene, food in zip(chromosome, self.food_items):
            factor = gene / 100.0 if food["unit_category"] == "Base Units" else gene
